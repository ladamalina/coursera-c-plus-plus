...
public:
  ...
  size_t Get(TAirport airport) const {
      return pairs_[static_cast<size_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
      pairs_[static_cast<size_t>(airport)]++;
  }
  ...
  void EraseAll(TAirport airport) {
      pairs_[static_cast<size_t>(airport)] = 0;
  }
  ...
  Items GetItems() const {
      ...
      for (size_t i = 0; i < pairs_.size(); ++i) {
          items[i] = {static_cast<TAirport>(i), pairs_[i]};
      }
      return items;
  };

private:
    array<size_t, SIZE> pairs_;
};
